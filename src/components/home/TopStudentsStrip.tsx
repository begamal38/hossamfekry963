...
export const TopStudentsStrip: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-y-0 left-0 w-8 md:w-24 bg-gradient-to-r from-primary/10 to-transparent pointer-events-none" />
      <div className="absolute inset-y-0 right-0 w-8 md:w-24 bg-gradient-to-l from-primary/10 to-transparent pointer-events-none" />
      
      {/* Header - Dynamic Month */}
      <div className="container mx-auto px-3 md:px-4 mb-3 md:mb-4 relative z-10">
        <div className="flex items-center justify-center gap-1.5 md:gap-2 text-center">
          <Trophy className="w-4 h-4 md:w-5 md:h-5 text-amber-500 flex-shrink-0" />
          <h3 className="text-xs md:text-base font-bold text-foreground whitespace-nowrap">
      ...
      <div 
        ...
        style={{ minHeight: isMobile ? '40px' : '48px' }}
      >
        {/* Gradient fade edges - Fixed for RTL/LTR */}
        <div 
          className={cn(
            "absolute inset-y-0 w-8 md:w-20 z-10 pointer-events-none",
        ...
        >
          {duplicatedStudents.map((student, idx) => (
            <div
              key={`${student.id}-${idx}`}
              className={cn(
                "inline-flex items-center gap-1.5 md:gap-2 px-2.5 md:px-4 py-1.5 md:py-2 rounded-full",
                "bg-gradient-to-r from-amber-500/10 to-primary/10",
                "border border-amber-500/20",
                "text-foreground text-[11px] md:text-sm font-medium",
